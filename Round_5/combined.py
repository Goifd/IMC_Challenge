...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for key in state.own_trades:
            # get list of trades for single product
            trades = state.own_trades[key]
            # iterate through list
            for trade in trades: 
                if trade.buyer == 'Vinnie':
                    best_bid, _ = list(state.order_depths[key].buy_orders.items())[0] 
                    # buy back from Valentina
                    result[key].append(Order(key, best_bid-5,-trade.quantity))

                # makes all the money atm
                if trade.seller == 'Valentina':
                    best_ask, _ = list(state.order_depths[key].sell_orders.items())[0]
                    # sell back to Valentina
                    result[key].append(Order(key, best_ask+5,trade.quantity))
